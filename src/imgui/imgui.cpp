 ...
 A: A primer on labels and the ID Stack...
   ...
   - Occasionally/rarely you might want change a label while preserving a constant ID. This allows
     ...
     but windows are uniquely identified by their ID. Use "###" to pass a label that isn't part of ID:
       ...
       sprintf(vbuf, "My game (%f FPS)###MyGame", fps);
       Begin(vbuf);            // Variable label,   ID = hash of "MyGame"
...
// B) When vbuf==NULL vsnprintf() will return the output size.
...
// NB: when active, hold on a privately held copy of the text (and apply back to 'vbuf'). So changing 'vbuf' while active has no effect.
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (focus_requested || user_clicked || user_scrolled || user_nav_input_start)
    {
        if (g.ActiveId != id)
        {
            ...
            // From the moment we focused we are ignoring the content of 'vbuf' (unless we are in read-only mode)
            ...
            // FIXME: We should probably compare the whole buffer to be on the safety side. Comparing vbuf (utf8) and edit_state.Text (wchar).
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // FIXME: We actually always render 'vbuf' when calling DrawList->AddText, making the comment above incorrect.
    ...
    // Render
    // Select which buffer we are going to display. When ImGuiInputTextFlags_NoLiveEdit is set 'vbuf' might still be the old value. We set vbuf to NULL to prevent accidental usage from now on.
